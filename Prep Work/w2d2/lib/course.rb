...
class Course
    attr_accessor :students, :add_student, :department, :credits, :conflicts_with
    attr_reader :name, :days, :time_block
    ...
    def conflicts_with?(course)
        ...
        self.days.each do |day|
            if course.days.include?(day)
                self.time_block == course.time_block ? conflicts += 1 : nil
            end
        end
        conflicts > 0 ? true : false
    end
end