...
class Array
  def my_uniq
    uniques = []
    self.each {|num| uniques.push(num) unless uniques.include?(num) }
    uniques
  end
end
...
class Array
  def median
    if self.length == 0
      ...
      new_arr = self.sort
      (new_arr[(self.length / 2) - 1] + new_arr[self.length / 2]) / 2.0
    else
      new_arr = self.sort
      new_arr[(self.length / 2)]
    end
  end
end
...
class Array
  def my_transpose
    ...
    i = 0
    j = 0
    while j < self[0].length && self[0][j] != nil
      while i < self.length
        new_arr[i] = self[i][j]
        i += 1
      end
