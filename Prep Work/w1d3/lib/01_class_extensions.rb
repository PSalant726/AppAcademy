...
class String
  def caesar(shift)
    ...
    i = 0
    while i < self.length
      self[i] == " " ? nums[i] = self[i].ord : nums[i] = self[i].ord + shift
      if nums[i] > 122
        nums[i] -= 26
      elsif nums[i] >= 90 && nums[i] < 97
        nums[i] -= 26
      end
      nums[i] = nums[i].chr unless nums[i] == " "
      i += 1
    end
    nums.join("")
  end
...
class Hash
  def difference(other_hash)
    hash_dif = {}
    self.each { |key, value| hash_dif[key] = value }
    other_hash.each do |key, value|
      hash_dif.has_key?(key) ? hash_dif.delete(key) : hash_dif[key] = value
    end
    hash_dif
  end
end
...
# Why does hash_dif = self modify the original hash?
...
class Fixnum
  def stringify(base)
    ...
    i = 0
    while base ** i <= self
    nums[i] = (self / (base ** i)) % base  
      if base != 16 || nums[i] <= 9
        string += nums[i].to_s
      elsif nums[i] > 9
        hex_hash = {10 => "a", 11 => "b", 12 => "c", 13 => "d", 14 => "e", 15 => "f"}
        string += hex_hash[nums[i]]
      end
      i += 1
    end
    string.reverse
